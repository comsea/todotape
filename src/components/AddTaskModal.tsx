...
interface AddTaskModalProps {
  ...
  todayKey: DayKey;
  onSave: (data: { name: string; day: DayKey; week: WeekKey; end: DayKey | null; endWeek: WeekKey | null; prio: 1 | 2 | 3; recurrence: 'none' | 'weekly'; recurId?: string }) => void;
  onClose: () => void;
}
...
export function AddTaskModal({ defaults, currentWeekDates, nextWeekDates, todayKey, onSave, onClose }: AddTaskModalProps) {
  ...
  const isEndDisabled = (d: DayMeta, endWk: WeekKey) => {
    ...
    if (endWk === week) return DAY_KEYS.indexOf(d.key) < dayIdx;
    return false;
  };
  ...
  const handleWeekChange = (w: WeekKey) => {
    ...
    if (end !== null && endWeek !== null && w === 'next' && endWeek === 'current') {
  ...
  const handleDayChange = (d: DayKey) => {
    setDay(d);
    if (end !== null && endWeek === week && DAY_KEYS.indexOf(end) < DAY_KEYS.indexOf(d)) {
      setEnd(null); setEndWeek(null);
    }
  };
  ...
  const submit = (e?: React.FormEvent) => {
    ...
    const recurId = recurring ? Math.random().toString(36).slice(2, 9) : undefined;
    onSave({
      name: name.trim(), day, week, end, endWeek, prio,
  ...
  return (
    <div className="modal-back" onClick={onClose}>
      <form className="modal cassette-big" onClick={(e) => e.stopPropagation()} onSubmit={submit}>
        ...
        <div className="modal-body">
          ...
          <div className="field">
            ...
            >
              ...
              <span className="recur-label">
                {recurring ? 'Se répète chaque semaine' : 'Une seule fois'}
              </span>
              <span className={`recur-dot ${recurring ? 'on' : ''}`} />
            </button>
            {recurring && (
              <div className="recur-hint">
                Cette tâche sera automatiquement recréée chaque semaine le même jour.
              </div>
            )}
          </div>
          ...
          <div className="field">
            ...
            <div className="day-picker" style={{ gap: 8 }}>
              {(['current', 'next'] as WeekKey[]).map(w => (
                <button key={w} type="button" className={`day-btn ${week === w ? 'on' : ''}`} style={{ minWidth: 90 }} onClick={() => handleWeekChange(w)}>
          ...
          <div className="field">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <div className="day-picker end-picker">
                <button type="button" className={`day-btn none ${end === null ? 'on' : ''}`} onClick={() => { setEnd(null); setEndWeek(null); }}>
